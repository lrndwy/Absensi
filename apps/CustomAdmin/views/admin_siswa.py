...
def admin_siswa(request):
    try:
        ...
        cache_key = f'siswa_data_{jenjang_filter}_{kelas_filter}_{today}'
        ...
        if table_data is None:
            ...
            # Subquery untuk mendapatkan status record_absensi terbaru
            ...
            for student in students:
                status = student[-4]
                status_verifikasi = student[-3]
                checktime = student[-2]
                tipe_absensi = student[-1]
            ...
            cache.set(cache_key, table_data, timeout=300)

        # Hapus cache jika ada perubahan data (POST request)
        if request.method == 'POST':
        ...
        context.update({
            ...
            'edit_data_siswa': edit_data_siswa if 'edit_data_siswa' in locals() else [],
