...
def admin_siswa(request):
    try:
        ...
        if print_id and start_date and end_date:
            try:
                ...
                context = {
                    ...
                    'API_LINK': reverse('api_siswa'),
                    'edit_url': reverse('admin_siswa'),
                    'siswa': True,
                }
        ...
        context.update({
            ...
            'API_LINK': reverse('api_siswa') + '?jenjang=' + request.GET.get('jenjang', '') + '&kelas=' + request.GET.get('kelas', ''),
            'print': True,
        })
...
def api_siswa(request):
    try:
        ...
        # Base queryset
        siswa_list = Siswa.objects.select_related('user', 'jenjang', 'kelas').annotate(
            today_status=Subquery(latest_absensi.values('status')),
            today_status_verifikasi=Subquery(latest_absensi.values('status_verifikasi')),
            today_tipe_absensi=Subquery(latest_absensi.values('tipe_absensi')),
            today_checktime=Subquery(latest_absensi.values('checktime'))
        ...
        if jenjang_filter:
            siswa_list = siswa_list.filter(jenjang__nama=jenjang_filter)
        if kelas_filter:
            siswa_list = siswa_list.filter(kelas__nama=kelas_filter)
